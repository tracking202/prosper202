...
if (!$_SESSION['stats202_limit'])
    $_SESSION['stats202_limit'] = 100;
if ($_SESSION['stats202_limit'] > 500)
    $_SESSION['stats202_limit'] = 500; //only allow a 500 limit on this view
if ($_SESSION['stats202_limit'])
...
<?php
if ($pages > 1) {
    $navBar .= " <span style='padding: 0px 10px'>&mdash;</span> ";
    if ($page > 1) {
...
switch ($_SESSION['stats202_order']) {
    case "statAccountNickName":
        if ($_SESSION['stats202_by'] == 'ASC') {
            $statAccountNickNameArrow = '&#9650;';
            $statAccountNickNameBy = 'DESC';
        } else {
            $statAccountNickNameArrow = '&#9660;';
            $statAccountNickNameBy = 'ASC';
        }
        break;
    case "subidActions":
        if ($_SESSION['stats202_by'] == 'ASC') {
...
<table cellpadding="0" cellspacing="0" class="offers-table">
    ...
    for ($x = 0; $x < count($subids); $x++) {
        ...
        if ($subids[$x]['subidAmount'] > 0) {
            $subids[$x]['subidAmount'] = '$' . number_format($subids[$x]['subidAmount'], 2);
        } else {
            $subids[$x]['subidAmount'] = '-';
        }

        $html = @array_map('htmlentities', $subids[$x]);
        if (strlen($html['statAccountNickName']) > 30) {
            $html['statAccountNickName'] = substr($html['statAccountNickName'], 0, 30) . '...';
        }
