...
<table cellspacing="0" cellpadding="0" class="legend">
    <tr>
        <th colspan="4"><h3 >Today's Stats - <?php echodate('g:ia'); ?></h3></th>
</tr>
...
if ($total_actions > 0) {
    $total_actions = number_format($total_actions);
} else {
    $total_actions = '-';
}
...
if ($total_epc > 0) {
    $total_epc = '$' . number_format($total_epc, 2);
} else {
    $total_epc = '-';
}
?>
