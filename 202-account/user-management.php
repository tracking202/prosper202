...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_user_id'])) {
	...
	$mysql['user_id'] = (string)$deleteUserId;
	$user_sql_delete = "UPDATE 202_users SET user_deleted = '1' WHERE user_id = " . $mysql['user_id'];
	if (!$db->query($user_sql_delete)) {
		p202_account_flash('bad', 'The user could not be removed; they can still sign in. Try again.');
		p202_account_redirect('202-account/user-management.php');
	}
	...
	foreach ($attributionCleanupQueries as $cleanupQuery) {
		try {
			// Each purge is best-effort (the user is already removed), but
			// a false return is logged like the exception it can be (#1).
			if (!$db->query($cleanupQuery) && function_exists('prosper_log')) {
				prosper_log('attribution_cleanup', $db->error);
			}
