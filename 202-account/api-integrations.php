...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['change_cb_key'] == '1') {
		...
		if ($_POST['cb_key'] == '') {
			...
			$error['cb_key'] = ($error['cb_key'] ?? '') . 'Clickbank Secret Key can\'t be empty!';
		}
	...
	if ($_POST['change_user_slack_incoming_webhook'] == '1') {
		...
		if ($_POST['user_slack_incoming_webhook'] == '') {
			...
			$error['user_slack_incoming_webhook'] = ($error['user_slack_incoming_webhook'] ?? '') . 'Slack Incoming Webhook URL can\'t be empty!';
		}
	...
	if ($_POST['change_user_revcontent_settings'] == '1') {
		if ($_POST['revcontent_user_id'] == '') {
			$error['user_revcontent'] = ($error['user_revcontent'] ?? '') . 'RevContent user ID can\'t be empty!<br>';
		}

		if ($_POST['revcontent_user_secret'] == '') {
			$error['user_revcontent'] = ($error['user_revcontent'] ?? '') . 'RevContent user secret can\'t be empty!<br>';
		}
...
<div class="row account">
	<div class="col-xs-12">
		<div class="row">
			...
			<div class="col-xs-8">
				<?php if (isset($change_jvzoo_secret_key) && $change_jvzoo_secret_key) { ?>
				...
				<?php } ?>
				<?php if (isset($error['ipqs_api_key_error']) && $error['ipqs_api_key_error']) { ?>
