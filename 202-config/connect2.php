 ...
 * Android intake.
 *
 * Expands EMPTY — never to the bare click id — when the click id is not a
 * canonical positive integer (the fallback redirect's "p202", a cloaked
 * placeholder) or the key is missing or unreadable: the install is then
 * recorded unattributed rather than attributable to a guessable click
 * (CLAUDE.md #11, #16). The key is read once per process and kept only when
 * it was read; one indexed primary-key lookup, made only for a URL that
 * carries the token.
 */
function p202InstallToken($clickId): string
{
    ...
    return \Api\V3\Apps\Android\InstallToken::expand($clickId, static function () use (&$key): ?string {
        if ($key === null) {
            $db = $GLOBALS['db'] ?? null;
...
{
    // The install token is computed from the RAW click id, before the
    // encoding below, and only when the URL asks for it. Its alphabet
    // (digits, ".", base64url) is unchanged by rawurlencode202().
    if (stripos((string) $url, '[[p202_install_token]]') !== false) {
        $installToken = null;
        if (isset($tokens['p202_install_token'])) {
            $installToken = (string) $tokens['p202_install_token'];
        } elseif (isset($tokens['subid'])) {
            $installToken = p202InstallToken($tokens['subid']);
        } elseif ($fillblanks) {
            $installToken = '';
        }
        if ($installToken !== null) {
            $url = str_ireplace('[[p202_install_token]]', $installToken, (string) $url);
        }
    }
    unset($tokens['p202_install_token']);
