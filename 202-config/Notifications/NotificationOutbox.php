 ...
 * (onEventMoved()). The full table is in plan §5.10.
 *
...
final class NotificationOutbox implements OutcomeNotificationSink
{
    ...
    public function onEventMoved(int $userId, int $newConvId, array $priorConvIds): bool
    {
        ...
        foreach ($announced as $row) {
            ...
            $this->conn->executeUpdate($cancel);
            $this->insert($userId, $newConvId, $pixelId, $destination, self::KIND_CORRECTION, 'suppressed', '',
                mb_strimwidth(self::NO_CORRECTION_URL . ' (the event was announced by conversion ' . implode(', ', $priorConvIds) . ')', 0, 255, '…', 'UTF-8'), $now);
        }
