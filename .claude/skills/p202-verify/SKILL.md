...
5. `go` — in this order, which is the script's order and matters for what a SKIP can hide:
   1. `gofmt -l .` with the installed toolchain, stdout and stderr kept apart. A listed file is FAIL, whatever else happens. A parse failure (exit 2, diagnostic on stderr) is FAIL when the local Go is CI's minor or newer, since Go never removes syntax; when the local Go is older, the decision is deferred to CI's own gofmt in step 2, and is SKIP if that toolchain cannot be fetched, because a syntax error and a version gap look the same from an older parser. After a toolchain switch in step 2, CI's own gofmt runs again even when the local one was clean, since a clean answer from another gofmt version is not CI's. If the installed toolchain ships no `gofmt` at all, the one bundled with CI's toolchain runs after step 2; when no switch happens and so no formatter ever ran, the tier is SKIP after vet and test (saying they passed), never PASS.
   2. Toolchain switch: when the local Go minor differs from the one `go-cli.yml` pins, `GOTOOLCHAIN` selects the latest patch of CI's minor (fetched on demand, about 15 seconds the first time) so a PASS means what CI's PASS means. If it cannot be resolved or fetched, SKIP, never a PASS with a footnote.
   3. `go list -m all` against the *committed* `go.mod` in a temp dir: if the baseline graph loads, the environment can fetch and any failure is the change's; if it fails with a network or proxy signature, SKIP, since nothing of this repository could have compiled.
   4. `cd go-cli && go vet ./...`, then `go test ./...`. When the output mentions the C toolchain, a probe builds a trivial cgo program *and* builds-and-runs a net-importing test in a temp module under the same toolchain; the host is broken if either fails (SKIP, saying whether vet had passed), otherwise the failure is the change (FAIL). Some hosts can vet under CI's toolchain but cannot run its test binaries (on the macOS this was written on, Go 1.22 test binaries that link `net` abort in dyld with `missing LC_UUID`); the tier then reads `SKIP  go vet passed under go1.22.12, but this host cannot run test binaries under it`.
   5. `go test ./cmd/...` with an isolated `HOME`, which catches flag checks that only pass because this machine has a CLI config. Only the CLI's configuration home is isolated: `GOENV`, `GOPATH`, `GOMODCACHE` and `GOCACHE` are resolved first and passed through (`GOENV` so settings kept with `go env -w`, such as `GOPROXY` or `GOFLAGS`, still apply), so the toolchain fetched in step 2 is not fetched again into a temp tree, and the temp home is removed afterwards. A failed `mktemp` is SKIP, not an unisolated PASS.
