...
final class PostbackReceiverIntegrationTest extends TestCase
{
    ...
    {
        self::$db->query(
            "INSERT INTO 202_app_registrations (user_id, platform, app_key, app_name, app_token, created_at, updated_at)
             VALUES (7, 'ios', '525463029', 'Owned app', 'integration-token', 1, 1)"
        );
        $registrationId = (int) self::$db->insert_id;
        $receiver = $this->receiver();
        ...
        $result = self::$db->query(
            'SELECT app_id, user_id, registration_id, protocol, signature_state, trusted, conversion_type, ad_interaction_type
             FROM 202_app_postbacks ORDER BY postback_id'
        );
        $rows = array_map(
        ...
        $this->assertSame(
            [
                ['app_id' => 525463029, 'user_id' => 7, 'registration_id' => $registrationId] + $expected,
                ['app_id' => 999000111, 'user_id' => 0, 'registration_id' => null] + $expected,
            ],
            $rows
        );
    }
    ...
    public function testAnAdAttributionKitDevelopmentPostbackIsTrustedOnlyOnceItsAppOptsIn(): void
    {
        ...
        self::$db->query(
            "INSERT INTO 202_app_registrations (user_id, platform, app_key, app_name, accept_test_signals, app_token, created_at, updated_at)
             VALUES (7, 'ios', '" . AdAttributionKitFixtures::EXAMPLE_APP_ID . "', 'Opted-in app', 1, 'integration-token-aak', 1, 1)"
        );
        ...
        $result = self::$db->query(
            'SELECT user_id, protocol, version, transaction_id, app_id, signature_state, trusted, key_id,
        ...
        $this->assertSame(
            ['user_id' => '0', 'trusted' => null, 'country_code' => 'US'] + $shared,
            ['user_id' => $rows[0]['user_id'], 'trusted' => $rows[0]['trusted'], 'country_code' => $rows[0]['country_code']] + array_intersect_key($rows[0], $shared),
            'before the registration: unclaimed, and nobody vouches for a development signature'
        );
        $this->assertSame(
            ['user_id' => '7', 'trusted' => '1', 'country_code' => 'CA'] + $shared,
            ['user_id' => $rows[1]['user_id'], 'trusted' => $rows[1]['trusted'], 'country_code' => $rows[1]['country_code']] + array_intersect_key($rows[1], $shared),
    ...
    private function postback(array $overrides = []): string
    {
        $encoded = json_encode($overrides + [
            ...
            // Not Apple's signature: stored flagged (trusted = 0),
    ...
    {
        $result = self::$db->query('SELECT COUNT(*) AS c FROM 202_app_postbacks');
