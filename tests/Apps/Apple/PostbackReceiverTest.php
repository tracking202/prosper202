...
use Api\V3\Apps\Apple\ParsedPostback;
...
final class PostbackReceiverTest extends TestCase
{
    ...
    private function retention(\mysqli $db): \Api\V3\Apps\AppRetention
    {
        return new \Api\V3\Apps\AppRetention($db, PostbackReceiver::retentionClasses());
    }
    ...
    private function capturedInsert(): ?array
    {
        foreach ($this->captured as $capture) {
            if (str_starts_with(ltrim($capture['sql']), 'INSERT INTO 202_app_postbacks')) {
                return $capture;
            }
        }
        return null;
    ...
    public function testValidPostbackIsStoredWithItsBoundValuesIntact(): void
    {
        ...
        $db = $this->capturingDb(['FROM 202_app_registrations' => [['registration_id' => 11, 'user_id' => 7, 'platform' => 'ios', 'app_key' => '525463029', 'accept_test_signals' => 0]]]);
        ...
        $this->assertSame(11, $row['registration_id'], 'the registration that claimed it, from the registry row');
    ...
    public function testUnregisteredAppStoresAsUnclaimed(): void
    {
        ...
        $db = $this->capturingDb(['FROM 202_app_registrations' => []]);
    ...
    public function testAPostbackNamingAppZeroNeverAsksTheRegistry(): void
    {
        ...
        // registration, so the receiver stores it unclaimed without a lookup.
        $postback = $this->signedV4Postback();
        $postback['app-id'] = 0;
        $rawBody = json_encode($postback);
        $this->assertNotFalse($rawBody);
    ...
    public function testADevelopmentSignatureIsTrustedOnlyWhenTheRegisteredAppOptedIn(array $appRows, int $expectedUserId, ?int $expectedTrustBit): void
    {
        ...
        $db = $this->capturingDb(['FROM 202_app_registrations' => $appRows]);
        ...
        $this->assertSame($expectedTrustBit, $row['trusted']);
    ...
    public static function developmentTrustCases(): array
    {
        return [
            ...
            'registered, opt-in off' => [[['registration_id' => 11, 'user_id' => 7, 'platform' => 'ios', 'app_key' => '525463029', 'accept_test_signals' => 0]], 7, null],
            'registered, opt-in on' => [[['registration_id' => 11, 'user_id' => 7, 'platform' => 'ios', 'app_key' => '525463029', 'accept_test_signals' => 1]], 7, 1],
            ...
            'registered, opt-in unreadable' => [[['registration_id' => 11, 'user_id' => 7, 'platform' => 'ios', 'app_key' => '525463029', 'accept_test_signals' => null]], 7, null],
        ];
    }
    ...
    public function testTheOptInNeverPromotesAForgedOrUnverifiableSignature(): void
    {
        ...
        $appRows = [['registration_id' => 11, 'user_id' => 7, 'platform' => 'ios', 'app_key' => '525463029', 'accept_test_signals' => 1]];
        foreach ([[SignatureState::INVALID, 0], [SignatureState::UNVERIFIABLE, null]] as [$state, $expected]) {
            ...
            $this->assertSame($expected, $row['trusted'], $state->value);
        }
    }
    ...
    public function testAProtocolCannotWriteAReceiverOwnedColumn(): void
    {
        ...
        try {
            ...
            $this->assertStringContainsString('trusted', $e->getMessage());
        }
    ...
    public function testAMalformedRetentionOverrideSkipsThatClassInsteadOfPruningOnTheDefault(): void
    {
        ...
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED=never');
        try {
            $db = $this->capturingDb();
            $this->retention($db)->prune(1_800_000_000);
            ...
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED');
        }
    }
    ...
    public function testRetentionPruningTargetsOnlyUnclaimedAndForgedRowsWithBoundedBatches(): void
    {
        ...
        $this->assertStringContainsString('WHERE trusted = 0 AND received_at < ?', $deletes[1]['sql']);
        $this->assertSame([$now - PostbackReceiver::DEFAULT_RETENTION_DAYS_REFUTED * 86400], $deletes[1]['values']);
        // The class an unauthenticated poster can mint for free: naming a
        // version the verifier does not know stores trusted = NULL,
        // and a body naming a registered app is claimed too — so without
        // this delete the row matched neither of the others and lived
        // forever.
        $this->assertStringContainsString('WHERE trusted IS NULL AND received_at < ?', $deletes[2]['sql']);
        $this->assertSame([$now - PostbackReceiver::DEFAULT_RETENTION_DAYS_UNVOUCHED * 86400], $deletes[2]['values']);
        foreach ($deletes as $delete) {
            $this->assertStringContainsString('LIMIT 500', $delete['sql'], 'a pass must stay cheap on the request path');
            $this->assertStringNotContainsString('trusted = 1', $delete['sql'], 'trusted rows are never pruned');
            $this->assertStringStartsWith('DELETE FROM 202_app_postbacks WHERE', $delete['sql']);
        }
    }

    public function testRetentionOverridesComeFromTheEnvironmentAndZeroDisablesAClass(): void
    {
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED=7');
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_REFUTED=0');
        try {
            $db = $this->capturingDb();
            $now = 1_800_000_000;
            $this->retention($db)->prune($now);
            ...
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED');
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_REFUTED');
        }
    }

    public function testRetentionPolicyReportsTheWindowsThisProcessResolved(): void
    {
        // What 202-cronjobs/app-retention.php prints. The overrides
        ...
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED=7');
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_REFUTED=0');
        putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNVOUCHED=90 days');
        try {
            $now = 1_800_000_000;
            $policy = $this->retention($this->capturingDb())->policy($now);

            $this->assertSame(['days' => 7, 'cutoff' => $now - 7 * 86400], $policy['postbacks/unclaimed']);
            $this->assertSame(['days' => 0, 'cutoff' => null], $policy['postbacks/refuted'], 'an explicit 0 disables the class');
            ...
            // value was refused.
            $this->assertSame(['days' => 0, 'cutoff' => null], $policy['postbacks/unvouched']);
        } finally {
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED');
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_REFUTED');
            putenv('P202_APP_RETENTION_DAYS_POSTBACKS_UNVOUCHED');
        }
    }
    ...
    public function testRetentionBacklogCountsEachClassAgainstItsOwnWindow(): void
    {
        ...
        $db = $this->capturingDb([
            ...
            'WHERE trusted = 0 AND' => [['aged' => 0]],
            'WHERE trusted IS NULL AND' => [['aged' => 1200]],
        ]);

        $backlog = $this->retention($db)->backlog($now);
        $this->assertSame(['postbacks/unclaimed' => 7, 'postbacks/refuted' => 0, 'postbacks/unvouched' => 1200], $backlog);
        ...
        $this->assertSame([$now - PostbackReceiver::DEFAULT_RETENTION_DAYS_REFUTED * 86400], $counts[1]['values']);
        $this->assertSame([$now - PostbackReceiver::DEFAULT_RETENTION_DAYS_UNVOUCHED * 86400], $counts[2]['values']);
    ...
    public function testRetentionBacklogRefusesToReadAMissingCountAsZero(): void
    {
        ...
        $this->retention($db)->backlog(1_800_000_000);
    }
    ...
    public function testADisabledClassCountsAsZeroWithoutAskingTheDatabase(): void
    {
        ...
        try {
            ...
            $backlog = $this->retention($db)->backlog(1_800_000_000);

            $this->assertSame(0, $backlog['postbacks/refuted'], 'a class that prunes nothing has no backlog');
