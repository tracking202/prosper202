 ...
 * @group integration
 */
final class AppReportIntegrationTest extends TestCase
{
    ...
    {
        self::$db->query(
            'INSERT INTO 202_app_registrations (user_id, platform, app_key, app_name, accept_test_signals, app_token, created_at, updated_at) VALUES '
            . '(' . self::OWNER . ", 'ios', '" . self::SKAN_APP_ID . "', 'SKAN app', 0, 'token-skan', 1, 1), "
            . '(' . self::OWNER . ", 'ios', '" . AdAttributionKitFixtures::EXAMPLE_APP_ID . "', 'AAK app', 1, 'token-aak', 1, 1)"
        );
        $aakRegistrationId = (int) self::$db->query(
            "SELECT registration_id FROM 202_app_registrations WHERE platform = 'ios' AND app_key = '" . AdAttributionKitFixtures::EXAMPLE_APP_ID . "'"
        )->fetch_assoc()['registration_id'];
    ...
    private function controller(): AppPostbacksController
    {
        return new AppPostbacksController(self::$db, self::OWNER);
    }
    ...
    public function testAReplayedPostbackCountsOnceAndReengagementsAreNeitherInstallsNorRedownloads(): void
    {
        ...
        $this->assertSame('trusted-only', $report['meta']['trusted'], 'the default report counts verified rows only');
        ...
        // counts, the trust-class _count columns included.
        ...
        $this->assertSame(1, $trusted['trusted_count']);
        $this->assertSame(2, $trusted['refuted_count'], 'three forged rows, two unique forged postbacks');
        $this->assertSame(0, $trusted['unvouched_count']);
        $this->assertSame(1, $trusted['test_count']);
    ...
    public function testTogglingTheDevelopmentOptInReflagsTheStoredRows(): void
    {
        ...
        $apps = new AppRegistrationsController(self::$db, self::OWNER);

        // Received under the opt-in: trusted.
        $this->assertSame(1, $this->onlyGroup($postbacks->report([]))['reengagements']);

        $apps->update($registrationId, ['accept_test_signals' => 0]);
        ...
        $this->assertSame(1, $off['unvouched_count']);
        $this->assertSame(1, $off['test_count'], 'the verdict itself never changes');

        $apps->update($registrationId, ['accept_test_signals' => 1]);
    ...
    public function testTheLegacySkadnetworkFilterNamesFindAdAttributionKitRowsToo(): void
    {
        ...
        $raw = self::$db->query(
            "SELECT redownload, fidelity_type FROM 202_app_postbacks WHERE transaction_id = 'AAK-REDOWNLOAD-1'"
        )->fetch_assoc();
    ...
    public function testAStoredJwsComesBackWholeForTheForensicPath(): void
    {
        ...
        $row = self::$db->query(
            "SELECT postback_id, signature_state FROM 202_app_postbacks WHERE transaction_id = 'AAK-LONG-JWS-1'"
        )->fetch_assoc();
        ...
        // The point of serving it: pasted into /apps/verify it
    ...
    public function testPostbacksThatDifferOnlyInWhereASeparatorFallsCountSeparately(): void
    {
        ...
        $this->assertSame(
            4,
            (int) self::$db->query('SELECT COUNT(*) AS c FROM 202_app_postbacks')->fetch_assoc()['c'],
        ...
        $this->assertSame(4, $group['refuted_count']);
