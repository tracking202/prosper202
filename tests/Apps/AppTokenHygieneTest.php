...
/**
 * The app token selects a registration on the pre-auth routes. It is an
 * identifier, not a secret — it ships inside every copy of the app binary —
 * but it is still kept to its two intended channels (the owner's scoped
 * GET/POST responses, and the X-P202-App-Token request header) and out of
 * logs, query strings and staged-change records: a token that leaks into a
 * log is one a rotation cannot take back from that log. Each of these was a
 * real finding (as the schema token, before the rename), and each regresses
 * silently because the happy path keeps working.
 */
final class AppTokenHygieneTest extends TestCase
{
    ...
    public function testAppCreateIsNotWrappedInIdempotencyReplay(): void
    {
        ...
        $start = strpos($src, "\$router->group('/apps',");
        self::assertIsInt($start, 'the /apps route group must be present');
    ...
    public function testPublicSchemaBranchAcceptsTheTokenOnlyAsAHeader(): void
    {
        ...
        // browser history. Extract the pre-auth /apps/schema branch and
        // assert it reads the header and nothing request-parameter shaped.
        $src = self::routerSource();
        $start = strpos($src, "\$path === '/apps/schema'");
        ...
        self::assertStringContainsString('RequestContext::header(\\Api\\V3\\Apps\\AppToken::HEADER_LOOKUP)', $branch);
        self::assertSame('x-p202-app-token', \Api\V3\Apps\AppToken::HEADER_LOOKUP);
        self::assertSame(strtolower(\Api\V3\Apps\AppToken::HEADER), \Api\V3\Apps\AppToken::HEADER_LOOKUP);
     ...
     */
    public function testApplyingAStagedChangeRedactsAppTokensFromTheResult(): void
    {
        $result = $this->applyWithResult('POST', '/apps', ['app_key' => '42'], [
            'registration_id' => 3,
            'app_token' => 'top-level-secret',
            'data' => [
                'app_token' => 'nested-secret',
                'apps' => [
                    ['app_key' => '42', 'app_token' => 'list-secret'],
                    ['app_key' => '43'],
                ],
            ],
        ]);
        ...
        self::assertSame('[redacted]', $result['app_token']);
        self::assertSame('[redacted]', $result['data']['app_token']);
        self::assertSame('[redacted]', $result['data']['apps'][0]['app_token']);
        // Redaction must not destroy the rest of the payload.
        self::assertSame(3, $result['registration_id']);
        self::assertSame('43', $result['data']['apps'][1]['app_key']);
    }
