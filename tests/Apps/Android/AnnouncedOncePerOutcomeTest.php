...
final class AnnouncedOncePerOutcomeTest extends TestCase
{
    ...
    {
        $this->clock += 5;
        (new GoalEngine(new Connection(self::$db), null, null, fn (): int => $this->clock, outbox: $outbox))
    ...
    public function testWithACorrectionUrlADeliveredRetractionIsAnsweredByACorrection(): void
    {
        ...
        $retracted = array_map(static fn (string $u): string => (string) preg_replace('/^.*(v=[^&]*&was=[^&]*).*$/', '$1', $u), $this->sent);
        sort($retracted);
        self::assertSame(['v=0.00&was=3.00', 'v=0.00&was=5.00'], $retracted, 'each retraction takes its outcome to 0');
        $this->sent = [];
        ...
        sort($this->sent);
        self::assertSame([
            'https://ts.example/fix?sub=100&v=3.00&was=0.00&conv=' . $bConv . '&k=correction',
            'https://ts.example/fix?sub=100&v=5.00&was=0.00&conv=' . $aV3 . '&k=correction',
