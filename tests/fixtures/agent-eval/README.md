...
a probe of the SKAN receiver URL, prints the REST API key), seeds this
...
(the URL Apple posts to) was a 404 on the workflow's PHP 8.3 while the same
...
the probe does not get a 200, and the SKAN cases' seeding `curl`s run with
