...
| `p202 app report` | The app report: Apple's postbacks with SKAN decoding by default (as before Android), Android installs with `--platform android`, both with `--platform all` (totals in `meta.totals`: `{ios, android, combined}`). Any platform groups by `day\|registration\|platform`; iOS also by `ad-network\|source\|country\|version\|protocol\|conversion-type`, counting unique trusted postbacks and every trust class beside them; Android also by `campaign\|match-state\|integrity-state\|goal` (the funnel), with `--match-state`, `--integrity-state`, `--trusted`, `--test`, `--aff-campaign-id`. A grouping or filter one platform lacks is refused before any request, naming the `--platform` to use. A day report with no `--time-from` covers the whole retained history: it returns the newest `--limit` populated days however far back they sit, and sets `meta.groups_truncated` when older ones were cut |
