...
| `entity_support` | object | Map of entity → object of supported operations. Each value is an object of booleans keyed by `list`, `get`, `create`, `update`, `delete`, and `bulk_upsert`. Entity keys are hyphenated: `aff-networks`, `ppc-networks`, `ppc-accounts`, `campaigns`, `landing-pages`, `text-ads`, `forecast-events`, `trackers`, `attribution-apps`, `attribution-conversion-values` (no `bulk_upsert`), and `attribution-postbacks` (read-only: `list` and `get`). |
...
| `features.attribution_postbacks` | array | The platform-signed attribution postback protocols this server receives, verifies and stores (`skadnetwork`, `adattributionkit`): Apple's SKAdNetwork receiver at `/.well-known/skadnetwork/report-attribution/` and AdAttributionKit receiver at `/.well-known/appattribution/report-attribution/`, the `/attribution` postback routes (postbacks, report, app registry, conversion-value rules, verify) under the `attribution` scope area, and the token-gated public `GET /attribution/schema` that iOS builds fetch at runtime. `p202 attribution` commands require it. See [Attribution postbacks](19-attribution-postbacks.md). |
