...
| `features.attribution_postbacks` | array | The platform-signed attribution postback protocols this server receives, verifies and stores (`skadnetwork` today): Apple's SKAdNetwork postback receiver at `/.well-known/skadnetwork/report-attribution/`, the `/attribution` postback routes (postbacks, report, app registry, conversion-value rules, verify) under the `attribution` scope area, and the token-gated public `GET /attribution/schema` that iOS builds fetch at runtime. `p202 attribution` commands require it. See [SKAdNetwork](19-attribution-postbacks.md). |
