...
- **Backups** — nearly all state is MySQL (`db_data` volume). Use Coolify's
  ...
  ```bash
  docker exec <db-container> sh -c 'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" prosper202' > prosper202-backup.sql
  ```
...
```bash
echo "SERVICE_PASSWORD_MYSQL=$(openssl rand -hex 16)" >> .env
