 ...
 * AdAttributionKit — and stores one row per postback in 202_app_postbacks.
 *
 ...
 * owning registration through the app registry (AppRegistry, keyed by
 * (ios, App Store id)), decides what the signature verdict is worth under
 * that registration's policy (Verdict::trustBit()), dedupes retries, stores
 * the row, and prunes what the open endpoint lets strangers mint.
 *
 * Response contract (what PostbackIntake sends):
 ...
 * `trusted` bit are stored on every row and surfaced through the reporting
 * API.
 */
final class PostbackReceiver
{
    ...
    /**
     * Retention defaults (days) for the rows the endpoint's openness makes
     * unbounded, registered with AppRetention (plan §4.6). Only a row that
     * is trusted and belongs to a registration is operator data kept
     * forever; every other class is something an unauthenticated poster can
     * mint, so each has a window:
     *
     *  - unclaimed  no registration adopted it (user_id = 0), including the
     *               rows a deleted user's purge released
     *  - refuted    the signature was checked and is forged (trusted = 0)
     *  - unvouched  the signature could not be checked at all, or was made
     *               with a development key the registration did not accept
     *               (trusted IS NULL)
     *
     * The third class is not a formality: naming a version outside
     * PostbackVerifier::VERIFIABLE_VERSIONS stores trusted = NULL, and if the
     * body names a registered App Store id (a public number) the row is
     * claimed too — so without this class it matched neither of the others
     * and lived forever, unauthenticated and free to repeat.
     *
     * Overridable per class via P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED /
     * _REFUTED / _UNVOUCHED; see AppRetention.
     */
    public const DEFAULT_RETENTION_DAYS_UNCLAIMED = 30;
    public const DEFAULT_RETENTION_DAYS_REFUTED = 90;
    public const DEFAULT_RETENTION_DAYS_UNVOUCHED = 90;

    /**
     * This source's retention classes, as AppRetention reads them.
     *
    ...
    {
        return [
            new RetentionClass('202_app_postbacks', 'unclaimed', 'user_id = 0', 'received_at', self::DEFAULT_RETENTION_DAYS_UNCLAIMED),
            new RetentionClass('202_app_postbacks', 'refuted', 'trusted = 0', 'received_at', self::DEFAULT_RETENTION_DAYS_REFUTED),
            new RetentionClass('202_app_postbacks', 'unvouched', 'trusted IS NULL', 'received_at', self::DEFAULT_RETENTION_DAYS_UNVOUCHED),
        ];
    }
    ...
    public const GENERIC_COLUMNS = [
        'user_id', 'registration_id', 'received_at', 'protocol', 'ad_network_id',
        'transaction_id', 'app_id', 'postback_sequence_index', 'did_win',
        'signature_state', 'trusted', 'key_id', 'dedupe_hash', 'raw_payload',
    ...
    public function receive(string $rawBody, string $remoteIp, ?int $receivedAt = null): array
    {
        ...
        try {
            // App Store ids are positive; a postback naming 0 (the protocols
            // accept any non-negative id) can belong to no registration.
            $registration = $parsed->appId >= 1
                ? (new AppRegistry($this->db))->byIdentity(AppIdentity::apple($parsed->appId))
                : null;
        } catch (\Throwable $e) {
            // A non-200 makes the device retry later, when the database may
            // be back — the postback is not lost.
            error_log('p202 apps: app registry lookup failed: ' . $e->getMessage());
            return $this->error(500, 'Failed to store postback');
        }
        // An unregistered app's postback is stored unclaimed and judged under
        // the untrusting policy; registering the app later claims it and
        // re-applies the registration's policy
        // (AppRegistrationsController::applyRegistrationPolicy()).
        $policy = $registration?->policy ?? AppPolicy::untrusting();
        ...
        $columns = [
            'user_id'                 => ['i', $registration?->userId ?? 0],
            'registration_id'         => ['i', $registration?->registrationId],
            ...
            'trusted'                 => ['i', $parsed->signatureState->trustBit($policy)],
        ...
        if ($insert === 'stored' && mt_rand(1, 100) === 1) {
            ...
            // 202-cronjobs/app-retention.php is the documented pruner —
            // deterministic, logged, and drained to completion. Never fatal
            // here: the accepted postback outcome stands regardless.
            try {
                (new AppRetention($this->db, self::retentionClasses()))->prune($receivedAt);
            } catch (\Throwable $e) {
                error_log('p202 apps: postback retention pruning failed: ' . $e->getMessage());
            }
        }
    ...
    {
        $sql = 'INSERT INTO 202_app_postbacks (' . implode(', ', array_keys($columns)) . ')'
        ...
        try {
            ...
            if (!$stmt) {
                error_log('p202 apps: postback INSERT prepare failed: ' . $this->db->error);
                return 'error';
            }
            // @phpstan-ignore-next-line the receiver is its own checked bind wrapper; no Connection in scope
            if (!$stmt->bind_param($types, ...$values)) {
                $stmt->close();
                error_log('p202 apps: postback INSERT bind failed');
                return 'error';
            }
            ...
            if (!$stmt->execute()) {
                ...
                error_log('p202 apps: postback INSERT failed: errno ' . $errno);
                return 'error';
            }
            $stmt->close();
            return 'stored';
            ...
            error_log('p202 apps: postback INSERT failed: ' . $e->getMessage());
            return 'error';
        }
    }
