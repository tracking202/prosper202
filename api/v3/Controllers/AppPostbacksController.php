...
/**
 * Read-only access to received app postbacks (SKAdNetwork and
 ...
 * the API never mutates them. A row belongs to the user whose registration
 * claimed it (registration_id); rows nobody has claimed (user_id = 0) become
 * visible once the app is registered — AppRegistrationsController claims
 * them. The postback's own `app_id` is kept as the forensic record of what
 * it named, and stays filterable.
 *
...
class AppPostbacksController
{
    ...
    private const SELECT_COLUMNS = 'postback_id, user_id, registration_id, received_at, protocol, version, ad_network_id, '
        ...
        . 'signature_state, trusted, key_id, remote_ip';

    /**
     * Most ids one `registration_ids` filter may carry.
     *
     ...
     * and tests/Apps/RegisteredAppsTest.php asserts the two agree — a
     * ceiling raised there and not here would make the nudge query a 422
     * that developmentNudges() swallows, and the nudges would simply stop
     * appearing.
     */
    private const MAX_REGISTRATION_IDS = 500;
    ...
    private const GROUP_MODES = [
        ...
        'registration'    => [['alias' => 'grp_registration_id', 'expr' => 'registration_id', 'key' => 'registration_id', 'kind' => 'nullable-int']],
    ...
    public function list(array $params): array
    {
        ...
        $countSql = "SELECT COUNT(*) as total FROM 202_app_postbacks $whereClause";
        ...
        // the tenant's.
        $sql = 'SELECT ' . self::SELECT_COLUMNS . " FROM 202_app_postbacks $whereClause"
    ...
    public function get(int $id): array
    {
        $sql = 'SELECT ' . self::SELECT_COLUMNS . ', attribution_signature'
            . ' FROM 202_app_postbacks WHERE postback_id = ? AND user_id = ? LIMIT 1';
        $stmt = $this->prepare($sql);
        ...
        // The signature is served for forensics (copy into /apps/verify, diff
        ...
        // back into /apps/verify. Bytes bound characters, so nothing
    ...
    /**
     * Aggregate postback report with conversion-value decoding.
     *
     * group_by: day (default, UTC), registration, ad-network, source, country,
     ...
     * trusted postbacks (trusted = 1) — the receiver stores forged and
     ...
     * They remain visible per group in the refuted/unvouched _count columns, and
    ...
    public function report(array $params): array
    {
        ...
        $trusted = $explicitSignature ? '1 = 1' : 'trusted = 1';
        ...
        $cvSql = 'SELECT ' . implode(', ', $selectGroup) . ",
            ...
            AND postback_id IN (
                SELECT MIN(postback_id) FROM 202_app_postbacks
                $cvWhereClause AND $trusted AND $winCondition
                GROUP BY $identity
            )
        ...
        if ($groupBy === 'registration') {
            $this->attachRegistrations($groupsOut);
        }
        ...
        return [
            ...
            'meta' => [
                'timezone' => 'UTC',
                'trusted' => $explicitSignature ? 'as-filtered' : 'trusted-only',
                'groups_truncated' => $truncated,
                'notes' => ($explicitSignature
                    ? 'metrics computed over the signature class the filter selected'
                    : 'installs/losses/decoding count trusted postbacks only (trusted = 1); refuted and unvouched rows appear in their own *_count columns')
    ...
    private static function metricColumns(
        ...
        // The trust classes every signal source shares (Verdict), and the
        // test signals beside them.
        $columns['trusted_count'] = $distinct('trusted = 1');
        $columns['refuted_count'] = $distinct('trusted = 0');
        $columns['unvouched_count'] = $distinct('trusted IS NULL');
        $columns['test_count'] = $distinct("signature_state = '$development'");
    ...
    private function reportTotals(array $where, array $binds, string $types, array $metrics): array
    {
        $sql = 'SELECT ' . self::metricSelectList($metrics)
            . ' FROM 202_app_postbacks WHERE ' . implode(' AND ', $where);
     ...
     * AppRegistrationsController::isUsableAppId() refuses the class the same
    ...
    private function buildFilters(array $params): array
    {
        ...
        foreach (['registration_id', 'app_id', 'campaign_id', 'postback_sequence_index'] as $param) {
            if (isset($params[$param]) && $params[$param] !== '') {
                $where[] = "$param = ?";
        ...
        // registration_ids narrows to a SET of registrations, which
        // registration_id alone cannot do. A caller that wants a per-app
        // answer about apps it already knows (the Setup page's test-signal
        // nudges) otherwise has to either ask once per app or group over
        // EVERYTHING and hope its own apps survive the LIMIT — and they do
        // not, because groups come back busiest first.
        //
        // Accepts a list or a comma string. Every element goes through the
        // same strictInt as registration_id: a value the int cast would MOVE
        // is refused rather than silently selecting a different app's rows.
        if (isset($params['registration_ids']) && $params['registration_ids'] !== '' && $params['registration_ids'] !== []) {
            ...
            if (!is_array($raw)) {
                if (!is_scalar($raw)) {
                    throw new ValidationException('Invalid filter value', [
                        'registration_ids' => 'Must be a list of registration ids, or a comma-separated string of them',
                    ]);
                }
            ...
            foreach ($raw as $element) {
                if (!is_scalar($element)) {
                    throw new ValidationException('Invalid filter value', [
                        'registration_ids' => 'Each registration id must be an integer',
                    ]);
                }
                $ids[] = self::strictInt(
                    ...
                    'Each registration id must be an integer'
                );
            }
            if (count($ids) > self::MAX_REGISTRATION_IDS) {
                throw new ValidationException('Invalid filter value', [
                    'registration_ids' => 'At most ' . self::MAX_REGISTRATION_IDS . ' registration ids',
                ]);
            }
            ...
            $where[] = 'registration_id IN (' . implode(', ', array_fill(0, count($ids), '?')) . ')';
            foreach ($ids as $id) {
                $binds[] = $id;
                $types .= 'i';
            }
        ...
        // counts (including development rows the registration accepts);
        ...
        if (isset($params['signature']) && $params['signature'] !== '') {
            ...
            $where[] = match ($signature) {
                SignatureState::VALID => 'trusted = 1',
                SignatureState::INVALID => 'trusted = 0',
                SignatureState::UNVERIFIABLE => 'trusted IS NULL',
    ...
    /**
     * Name each registration group: app_name, platform and app_key beside
     * its id. A group whose registration is gone (its postbacks were
     * unlinked when it was deleted) carries nulls.
     *
     * @param array<string, array<string, mixed>> $groupsOut
     */
    private function attachRegistrations(array &$groupsOut): void
    {
        $sql = 'SELECT registration_id, app_name, platform, app_key FROM 202_app_registrations WHERE user_id = ?';
        $stmt = $this->prepare($sql);
        $this->bind($stmt, 'i', $this->userId);
        $this->execute($stmt, 'Registration names query failed');
        $result = $this->result($stmt);
        $registrations = [];
        while ($row = $result->fetch_assoc()) {
            $registrations[(int)$row['registration_id']] = $row;
        }
        $stmt->close();

        foreach ($groupsOut as &$group) {
            $registration = $registrations[(int)($group['registration_id'] ?? 0)] ?? null;
            $group['app_name'] = $registration === null ? null : ResponseSanitizer::cleanVisitorString((string)$registration['app_name']);
            $group['platform'] = $registration === null ? null : (string)$registration['platform'];
