...
///     P202SKAN_LIVE_TOKEN=<schema token from POST /skan/apps> \
///     swift test --filter LiveServerIntegrationTests
final class LiveServerIntegrationTests: XCTestCase {
    private var endpoint: URL? {
        guard let raw = ProcessInfo.processInfo.environment["P202SKAN_LIVE_ENDPOINT"] else {
            return nil
        }
    ...
    private var token: String? {
        return ProcessInfo.processInfo.environment["P202SKAN_LIVE_TOKEN"]
    }

    func testFetchesDecodesAndConditionallyRefetchesTheSchema() throws {
        guard let endpoint, let token else {
            throw XCTSkip("Set P202SKAN_LIVE_ENDPOINT and P202SKAN_LIVE_TOKEN to run against a live instance")
        }

        let sdk = P202SKAN(store: InMemoryStore())
        var firstSchema: P202SKANSchema?
    ...
    func testWrongTokenIsSurfacedAsAnHTTPStatusNotACrash() throws {
        ...
        sdk.refreshSchema { result in
            if case let .failure(error) = result {
                XCTAssertEqual(error as? P202SKAN.SDKError, .unexpectedStatus(404))
            } else {
