...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($login_log_stmt) {
		...
		if (!$login_log_stmt->execute()) {
			prosper_log('login', 'Unable to record login attempt: ' . $login_log_stmt->error);
		}
	...
	if (empty($error) && $user_row) {
		...
		if ($update_stmt) {
			...
			if (!$update_stmt->execute()) {
				prosper_log('login', 'Unable to update last-login state for user_id ' . $user_id . ': ' . $update_stmt->error);
			}
			$update_stmt->close();
		} else {
			// MySQL rejects a denied privilege at prepare, so this is the
			// branch that fires when 202_users is not writable.
			prosper_log('login', 'Unable to prepare last-login update: ' . $db->error);
		}
