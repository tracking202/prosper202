...
/**
 * App measurement retention cronjob. Run hourly (or daily).
 *
 * The public app intakes — /.well-known/skadnetwork/report-attribution/ and
 * its AdAttributionKit sibling today — are open, unauthenticated endpoints:
 * devices cannot present credentials, so anyone can post. Rows nobody will
 * ever act on therefore have windows, registered by each signal source
 * (Api\V3\Apps\AppRetention): for postbacks, unclaimed rows no registration
 * adopted, refuted rows whose signature verified as forged, and unvouched
 * rows nobody could vouch for. Trusted rows belonging to a registration are
 * never touched.
 *
 ...
 *                     removes up to AppRetention::PRUNE_BATCH_LIMIT rows per
 *                     class, so the
 ...
 *   P202_APP_RETENTION_DAYS_POSTBACKS_UNCLAIMED  (default 30)
 *   P202_APP_RETENTION_DAYS_POSTBACKS_REFUTED    (default 90)
 *   P202_APP_RETENTION_DAYS_POSTBACKS_UNVOUCHED  (default 90)
 ...
 *   17 * * * * /usr/bin/php /path/to/prosper202/202-cronjobs/app-retention.php \
 *     >> /var/log/prosper202/app-retention.log 2>&1
 */
...
$warn = static function (string $message): void {
    ...
    if (defined('STDERR')) {
        fwrite(STDERR, 'app-retention: ' . $message . "\n");
        return;
    }
    error_log('app-retention: ' . $message);
};
...
try {
    // An install upgraded from before 1.9.76 has no app tables, and a
    ...
    $needed = (int) ceil($backlog / AppRetention::PRUNE_BATCH_LIMIT);

    if ($dryRun) {
        echo 'app-retention completed (dry run: nothing deleted, '
...
echo "app-retention completed ({$passes} pass(es))\n";
