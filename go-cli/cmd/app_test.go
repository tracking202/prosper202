...
func TestAppPostbacksListMapsFlagsToApiParams(t *testing.T) {
	var gotPath string
	...
	_, _, err := executeCommand("app", "postbacks", "list",
	...
	if err != nil {
		t.Fatalf("app postbacks list error: %v", err)
	}
	if !strings.HasSuffix(gotPath, "/apps/postbacks") {
		t.Errorf("path = %q, want .../apps/postbacks", gotPath)
	}
...
func TestAppReportLiftsGroupsIntoTheListShape(t *testing.T) {
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
...
func TestAppVerifySendsThePayloadWithIntegersIntact(t *testing.T) {
	var gotBody []byte
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
	...
	stdout, _, err := executeCommand("app", "verify", "--file", file, "--json")
	if err != nil {
		t.Fatalf("app verify error: %v", err)
	}
...
func TestAppVerifyRejectsNonJsonInputWithAHint(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
...
func TestAppEncodingCreateDemandsExactlyOneValueKind(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
	writeTestConfig(t, tmp, "http://127.0.0.1:0", "test-key")
...
func TestAppCreateRequiresExactlyOneAppNameAndAName(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
	writeTestConfig(t, tmp, "http://127.0.0.1:0", "test-key")

	for _, args := range [][]string{
		{"app", "create", "--app-name", "My App"},
		{"app", "create", "--app-name", "My App", "--app-key", "42", "--store-link", "https://apps.apple.com/us/app/x/id42"},
	} {
		_, _, err := executeCommand(args...)
		if err == nil || !strings.Contains(err.Error(), "--store-link") || !strings.Contains(err.Error(), "--app-key") {
			t.Fatalf("%v: expected both naming flags to be named, got %v", args, err)
		}
		// The category is the contract, not the prose: a bare fmt.Errorf here
		// would still name the flags, but the --json envelope would report no
		// category of its own and the hint would be gone.
		assertValidationError(t, err)
		if hint := hintFor(err); !strings.Contains(hint, "apps.apple.com") {
			t.Errorf("hint should show a link to paste, got %q", hint)
		}
	}
	_, _, err := executeCommand("app", "create", "--app-key", "42")
...
func TestAppCreateSendsTheStoreLinkAndTheTestSignalPolicy(t *testing.T) {
	...
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(201)
		w.Write([]byte(`{"data":{"registration_id":7,"platform":"android","app_key":"com.example.app","app_name":"My App","accept_test_signals":1}}`))
	}))
	defer srv.Close()
	...
	_, _, err := executeCommand("app", "create", "--store-link", "https://play.google.com/store/apps/details?id=com.example.app",
		"--app-name", "My App", "--accept-test-signals", "1")
	if err != nil {
		t.Fatalf("app create error: %v", err)
	}
	if !strings.HasSuffix(gotPath, "/apps") {
		t.Errorf("path = %q, want .../apps", gotPath)
	}
	// The server reads the link; the CLI forwards it untouched rather than
	// parsing it into a second implementation.
	if got := gotBody["store_link"]; got != "https://play.google.com/store/apps/details?id=com.example.app" {
		t.Errorf("store_link = %v", got)
	}
	if _, sent := gotBody["app_key"]; sent {
		t.Errorf("app_key sent although only --store-link was given: %v", gotBody)
	}
	if got := gotBody["accept_test_signals"]; got != "1" {
		t.Errorf("accept_test_signals = %v, want \"1\"", got)
	}
}

func TestAppCreateRejectsANonBooleanPolicyAndAnUnknownPlatform(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
	writeTestConfig(t, tmp, "http://127.0.0.1:0", "test-key")

	_, _, err := executeCommand("app", "create", "--app-key", "42", "--app-name", "My App", "--accept-test-signals", "yes")
	if err == nil || !strings.Contains(err.Error(), "--accept-test-signals") {
	...
	_, _, err = executeCommand("app", "create", "--app-key", "42", "--app-name", "My App", "--platform", "windows")
	if err == nil || !strings.Contains(err.Error(), "ios, android") {
		t.Fatalf("expected the platforms to be listed, got %v", err)
	}
	assertValidationError(t, err)
	if hint := hintFor(err); !strings.Contains(hint, "--platform out") {
		t.Errorf("hint = %q, want it to say the platform can be inferred", hint)
	}
}

func TestAppUpdateDoesNotOfferTheFlagsThatNameTheApp(t *testing.T) {
	// A registration's app is fixed; offering --app-key on update would
	// promise a change the server refuses.
	for _, flag := range []string{"store-link", "app-key", "platform"} {
		if appUpdateCmd.Flags().Lookup(flag) != nil {
			t.Errorf("app update registers --%s", flag)
		}
		if appCreateCmd.Flags().Lookup(flag) == nil {
			t.Errorf("app create lacks --%s", flag)
		}
	}
}

func TestAppEncodingNamesTheCommandForARefusedRegistration(t *testing.T) {
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(422)
		w.Write([]byte(`{"error":true,"message":"Invalid registration_id","status":422,"field_errors":{"registration_id":"Registration 9 is an Android app; SKAN encodings apply to iOS apps only."}}`))
	}))
	defer srv.Close()
	...
	if code := exitCodeForError(err); code != ExitValidation {
		t.Errorf("exit code = %d, want %d (a 422 is the caller's input)", code, ExitValidation)
	}
	if hint := hintFor(err); !strings.Contains(hint, "p202 app list --platform ios") {
		t.Errorf("hint = %q, want it to name the command that lists iOS registrations", hint)
	}
}

func TestAppRotateTokenPostsToTheRotateRoute(t *testing.T) {
	var gotMethod, gotPath string
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		gotMethod = r.Method
		gotPath = r.URL.Path
		w.WriteHeader(200)
		w.Write([]byte(`{"data":{"registration_id":7,"app_id":525463029,"app_token":"` + strings.Repeat("ab", 32) + `"}}`))
	}))
	defer srv.Close()
	...
	stdout, _, err := executeCommand("app", "rotate-token", "7", "--json")
	if err != nil {
		t.Fatalf("app rotate-token error: %v", err)
	}
	if gotMethod != "POST" || !strings.HasSuffix(gotPath, "/apps/7/app-token/rotate") {
		t.Errorf("request = %s %s, want POST .../apps/7/app-token/rotate", gotMethod, gotPath)
	}
	if !strings.Contains(stdout, "app_token") {
		t.Errorf("stdout should carry the new token, got:\n%s", stdout)
	}
}

func TestAppSchemaFetchesTheDeviceFacingDocumentByHeaderToken(t *testing.T) {
	token := strings.Repeat("cd", 32)
	var schemaHeaderToken, schemaRawQuery string
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case strings.HasSuffix(r.URL.Path, "/apps/7"):
			w.WriteHeader(200)
			w.Write([]byte(`{"data":{"registration_id":7,"app_id":525463029,"app_token":"` + token + `"}}`))
		case strings.HasSuffix(r.URL.Path, "/apps/schema"):
	...
	stdout, _, err := executeCommand("app", "schema", "7", "--json")
	if err != nil {
		t.Fatalf("app schema error: %v", err)
	}
	if schemaHeaderToken != token {
		t.Errorf("schema fetched with X-P202-App-Token %q, want the app's %q", schemaHeaderToken, token)
	}
...
func TestAppSchemaWithoutATokenNamesTheServerRequirement(t *testing.T) {
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		w.Write([]byte(`{"data":{"registration_id":7,"app_id":525463029}}`))
	}))
	defer srv.Close()
	...
	_, _, err := executeCommand("app", "schema", "7")
	if err == nil {
		t.Fatal("expected a validation error when the registration has no token")
	}
	assertValidationError(t, err)
	if hint := hintFor(err); !strings.Contains(hint, "features.app_platforms") {
		t.Errorf("hint should name the server requirement, got %q", hint)
	}
}

func TestAppDeleteDryRunPreviewsWithoutConfirmation(t *testing.T) {
	var gotMethod, gotPath string
	var gotParams url.Values
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		gotMethod = r.Method
		...
		gotParams = r.URL.Query()
		w.WriteHeader(200)
		w.Write([]byte(`{"data":{"dry_run":true,"action":"delete","resource":"202_app_registrations","mode":"hard","record":{"registration_id":7},"cascade":[]}}`))
	}))
	defer srv.Close()
	...
	stdout, _, err := executeCommand("app", "delete", "7", "--dry-run")
	if err != nil {
		t.Fatalf("app delete --dry-run error: %v", err)
	}
	if gotMethod != "DELETE" || !strings.HasSuffix(gotPath, "/apps/7") {
		t.Errorf("request = %s %s, want DELETE .../apps/7", gotMethod, gotPath)
	}
...
func TestAppVerifyUnderStagedModeStaysAnImmediateRead(t *testing.T) {
	...
	stdout, _, err := executeCommand("app", "verify", "--file", file, "--staged", "--json")
	if err != nil {
		t.Fatalf("app verify --staged error: %v", err)
	}
...
func TestAppEncodingUpdateClearFlagsSendExplicitNulls(t *testing.T) {
	...
	_, _, err := executeCommand("app", "encoding", "update", "5", "--clear-fine-value", "--coarse-value", "high", "--json")
	if err != nil {
		t.Fatalf("app encoding update error: %v", err)
	}
	if gotMethod != "PUT" {
...
func TestAppEncodingUpdateClearAndSetOfTheSameKindConflict(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
	// No server: the conflict is rejected before any request is built.
	writeTestConfig(t, tmp, "http://127.0.0.1:0", "test-key")

	_, _, err := executeCommand("app", "encoding", "update", "5", "--clear-fine-value", "--fine-value", "10")
	if err == nil {
		t.Fatal("expected a validation error")
	}
	assertValidationError(t, err)
	// The coarse pair is the analogous site; both must answer alike.
	_, _, err = executeCommand("app", "encoding", "update", "5", "--clear-coarse-value", "--coarse-value", "high")
	if err == nil {
...
func TestAppListAllPaginatesThroughEveryPage(t *testing.T) {
	...
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(200)
		if offset == "0" {
			w.Write([]byte(`{"data":[{"registration_id":1,"app_id":100},{"registration_id":2,"app_id":200}],"pagination":{"total":3,"limit":2,"offset":0}}`))
			return
		}
		w.Write([]byte(`{"data":[{"registration_id":3,"app_id":300}],"pagination":{"total":3,"limit":2,"offset":2}}`))
	}))
	defer srv.Close()
	...
	stdout, _, err := executeCommand("app", "list", "--all", "--json")
	if err != nil {
		t.Fatalf("app list --all error: %v", err)
	}
...
func TestAppPagingFlagsAreValidatedOnEveryCommandThatTakesThem(t *testing.T) {
	...
	cases := []struct {
		...
		{"postbacks list, non-numeric limit", []string{"app", "postbacks", "list", "--limit", "abc"}, "--limit must be a positive integer", "--limit 50"},
		{"app list, non-numeric limit", []string{"app", "list", "--limit", "abc"}, "--limit must be a positive integer", "--limit 50"},
		{"cv list, non-numeric limit", []string{"app", "encoding", "list", "--limit", "abc"}, "--limit must be a positive integer", "--limit 50"},
		{"report, non-numeric limit", []string{"app", "report", "--limit", "abc"}, "--limit must be a positive integer", "--limit 50"},
		{"postbacks list, zero limit", []string{"app", "postbacks", "list", "--limit", "0"}, "--limit must be a positive integer", "--limit 50"},
		{"postbacks list, negative offset", []string{"app", "postbacks", "list", "--offset", "-5"}, "--offset must be a non-negative integer", "--all"},
		{"app list, non-numeric offset", []string{"app", "list", "--offset", "x"}, "--offset must be a non-negative integer", "--all"},
		{"cv list, negative offset", []string{"app", "encoding", "list", "--offset", "-1"}, "--offset must be a non-negative integer", "--all"},
	}
...
func TestAppListsStillSendValidatedPagingThrough(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"postbacks", "/apps/postbacks", []string{"app", "postbacks", "list", "--limit", "5", "--offset", "10"}},
		{"apps", "/apps", []string{"app", "list", "--limit", "5", "--offset", "10"}},
		{"conversion values", "/apps/skan-encodings", []string{"app", "encoding", "list", "--limit", "5", "--offset", "10"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			var gotPath string
			var gotParams url.Values
...
func TestAppListsKeepTheApiErrorContract(t *testing.T) {
	for _, tc := range []struct {
		name string
		args []string
	}{
		{"postbacks", []string{"app", "postbacks", "list"}},
		{"apps", []string{"app", "list"}},
		{"conversion values", []string{"app", "encoding", "list"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
				w.WriteHeader(401)
...
func TestAppUpdatesUseTheSameEmptyBodyWordingAsTheGeneratedCommands(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"app update", []string{"app", "update", "7"}, "--app-name"},
		{"cv update", []string{"app", "encoding", "update", "5"}, "--fine-value"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, _, err := executeCommand(tc.args...)
			if err == nil {
...
func TestAppVerifyWithoutPipedInputFailsInsteadOfBlocking(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"no --file", []string{"app", "verify", "--json"}},
		{"--file -", []string{"app", "verify", "--file", "-", "--json"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, _, err := executeCommand(tc.args...)
			if err == nil {
...
func TestAppVerifyStillReadsAPipedPostback(t *testing.T) {
	var gotBody []byte
	srv := httptest.NewServer(withCapabilities(func(w http.ResponseWriter, r *http.Request) {
		gotBody, _ = io.ReadAll(r.Body)
		w.WriteHeader(200)
	...
	stdout, _, err := executeCommand("app", "verify", "--json")
	if err != nil {
...
func TestAppVerifyNamesAnUnreadableFile(t *testing.T) {
	tmp := t.TempDir()
	setTestHome(t, tmp)
	writeTestConfig(t, tmp, "http://127.0.0.1:0", "test-key")

	_, _, err := executeCommand("app", "verify", "--file", filepath.Join(tmp, "nope.json"))
	if err == nil {
...
func TestAppSchemaSeparatesAnUnreadableResponseFromAMissingToken(t *testing.T) {
