...
//show real or filtered clicks
	...
	if ($user_row['user_cpc_or_cpv'] == 'cpv')  $cpv = true;
		...
		//lets build the new keyword list
			...
			$query = query('
						SELECT 
							2ca.city_id AS city_id,
							2ci.city_name AS city_name,
							2cy.country_code AS country_code, 
							COUNT(*) AS clicks, 
						...
						LEFT OUTER JOIN 202_locations_country AS 2cy ON (2cy.country_id = 2ci.main_country_id)
			', $db_table, true, true, false, " $click_filtered GROUP BY city_id ORDER BY clicks DESC", $_POST['offset'], true, true);
			...
			<div class="row" style="margin-top: 10px;">
				...
				<div class="col-xs-6 text-right" style="top: -10px;">
					...
					<a style="font-size:12px;" target="_new" href="/tracking202/analyze/cities_download.php">
			...
			<table class="table table-bordered table-hover" id="stats-table">
				...
				<tr style="background-color: #f2fbfa;">
					<th colspan="2" style="text-align:left">City</th>
					<th>Clicks</th>
			...
			if ($total_rows > 0) {
				...
				while ($click_row = $info_result->fetch_assoc()) {
					...
					//roi
						...
						<tr>
							<td colspan="2" style="text-align:left; padding-left:10px;"><img src="/202-img/flags/<?php echo strtolower($html['country_code']);?>.png"> <?php echo $html['city_name'];?> (<?php echo $html['country_code'];?>)</td>
...
<div class="col-xs-12 text-center">
	<div class="pagination" id="table-pages">
	    <ul>
			<?if ($query['offset'] > 0) {
					printf(' <li class="previous"><a class="fui-arrow-left" onclick="loadContent(\'/tracking202/ajax/sort_cities.php\',\'%s\',\'%s\');"></a></li>', $i, $html['order']);
				}
				...
				if ($query['pages'] > 1) {
					for ($i=0; $i < $query['pages']-1; $i++) {
						if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) {
							...
							printf(' <li %s><a onclick="loadContent(\'/tracking202/ajax/sort_cities.php\',\'%s\',\'%s\');">%s</a></li>', $class, $i, $html['order'], $i+1);
							unset($class);
						}
					}
				}
				...
				if ($query['offset'] > 0) {
					printf(' <li class="next"><a class="fui-arrow-right" onclick="loadContent(\'/tracking202/ajax/sort_cities.php\',\'%s\',\'%s\');"></a></li>', $query['offset'] - 1, $html['order']);
				}
			?>
		</ul>
	</div>
