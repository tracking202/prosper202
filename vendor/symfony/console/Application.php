...
class Application
{
    ...
    public function add(Command $command)
    {
        ...
        if (!$command->getName()) {
            throw new LogicException(sprintf('The command defined in "%s" cannot have an empty name.', $command::class));
        }
    ...
    public function findNamespace($namespace)
    {
        ...
        $expr = preg_replace_callback('{([^:]+|)}', fn($matches) => preg_quote((string) $matches[1]).'[^:]*', $namespace);
    ...
    public function find($name)
    {
        ...
        $expr = preg_replace_callback('{([^:]+|)}', fn($matches) => preg_quote((string) $matches[1]).'[^:]*', $name);
    ...
    protected function doRenderException(\Exception $e, OutputInterface $output)
    {
        do {
            ...
            if ('' === $message || OutputInterface::VERBOSITY_VERBOSE <= $output->getVerbosity()) {
                $title = sprintf('  [%s%s]  ', $e::class, 0 !== ($code = $e->getCode()) ? ' ('.$code.')' : '');
    ...
    private function splitStringByWidth($string, $width)
    {
        ...
        if (false === $encoding = mb_detect_encoding((string) $string, null, true)) {
            return str_split((string) $string, $width);
        }
